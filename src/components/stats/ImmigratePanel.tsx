...
if (userLang.startsWith('en')) {
  MIGRATE_WARNING = MIGRATE_EN
  RELOAD = RELOAD_EN
} else if (userLang.startsWith('fr')) {
  MIGRATE_WARNING = MIGRATE_FR
  RELOAD = RELOAD_FR
}
...
export const ImmigratePanel = () => {
  ...
  const handleSaveButton = () => {
    ...
    if (textarea && window.confirm(MIGRATE_WARNING)) {
      ...
      if (migrationStats.unlimitedState) {
        saveUnlimitedGameStateToLocalStorage(migrationStats.unlimitedState)
      }
      ...
      if (migrationStats.unlimitedStatistics) {
        saveUnlimitedStatsToLocalStorage(migrationStats.unlimitedStatistics)
      }
